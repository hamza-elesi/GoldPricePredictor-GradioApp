...
Forest_regressor_model = joblib.load(model_filename)
...
def predict_gld_price(SPX, USO, SLV, EURUSD):
    ...
    predicted_price = Forest_regressor_model.predict(features_df)
    return float(predicted_price[0])  # Convert NumPy float to Python float

# Define the Gradio interface
def main():
    with gr.Blocks() as demo:
        gr.Markdown("## Gold Price Prediction Model 📈 Using Random Forest Regressor")
